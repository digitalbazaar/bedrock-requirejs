...
api.readBowerPackagesSync = function() {
  ...
  bedrock.config.requirejs.bower.packages.forEach(function(custom) {
    ...
    if(typeof custom.manifest === 'string') {
      ...
      throw new TypeError(
        'Pseudo bower package `manifest` must be a bower.json filename.');
    }
    var pkg = {
      path: custom.path,
      manifest: manifest,
...
function _getBowerManifestFile(pkg) {
  ...
  // check for bower.json, then package.json, .bower.json, components.json
  var filename;
  ['bower.json', 'package.json', '.bower.json', 'components.json']
    .forEach(type => {
      if(filename) {
        return;
      }
...
function _initPackage(pkg, pkgs) {
  ...
  if(!('manifestUrl' in pkg)) {
    pkg.manifestUrl = bedrock.config.requirejs.bower.componentsUrl + '/' +
      pkg.manifest.name + '/' + path.basename(pkg.manifestPath);
  }
  ...
  pkg.findMainFiles = function(options) {
    ...
    if(rval.length === 0 && options.extension === '.js') {
      // try default names
      if(fs.existsSync(pkg.path + '/main.js')) {
        rval.push('./main.js');
      } else if(fs.existsSync(pkg.path + '/' + pkg.manifest.name + '.js')) {
